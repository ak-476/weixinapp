var des3iv = '12345678';
var DES3_KEY = "YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4";
var DES3 = {
  ...
  decrypt: function(key, input) {
    var genKey = genkey(key, 0, 24);
    return des(genKey.key, window.atob(input), 0, 0, des3iv, 1)
  }
};
  ...
  if (!encrypt) {
    if (padding === 1) {
      var len = result.length,
        paddingChars = 0;
      len && (paddingChars = result.charCodeAt(len - 1)); (paddingChars <= 8) && (result = result.substring(0, len - paddingChars))
    }
    try {
...
export default {
  des3iv,DES3_KEY,DES3,des,des_createKeys,genkey,pad
}
