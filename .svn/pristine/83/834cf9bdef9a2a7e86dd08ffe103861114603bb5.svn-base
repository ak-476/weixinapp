...
,      }
    ...
    methods:{
      ...
      secondMenuCheck(o){
        ...
        this.$router.push({name: 'cyclopeResult', query: {checkId: o.id,firstId: this.firstMenuId, pageSize: 10, type: this.$route.query.type}});
      },
      search(){
        if(this.cancal == '取消'){
        ...
        this.showTabList = false;
        this.$router.push({name: 'cyclopeResult', query:{ keyword: this.keyword, checkId: -1, pageSize: 10, type: this.$route.query.type}})
        this.keyword = '';
      },
      getTabList(){
